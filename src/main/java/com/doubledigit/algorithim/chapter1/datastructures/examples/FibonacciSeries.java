...
public class FibonacciSeries {
    ...
    public static void main(String[] args) {
        UnaryOperator<Pair<BigInteger, BigInteger>> fib = pair -> new Pair<>(pair.getValue(), pair.getKey().add(pair.getValue()));

        String fibonacci = Stream.iterate(base, fib)
                .map(pair -> pair.getKey()).limit(20)
                .map(bigInteger -> String.valueOf(bigInteger))
